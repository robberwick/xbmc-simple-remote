// Generated by CoffeeScript 1.4.0
(function() {
  var XbmcAdapter, buttons;

  buttons = {
    Player: ['PlayPause', 'Stop'],
    Input: ['Up', 'Down', 'Left', 'Right', 'Select', 'Back', 'Home']
  };

  XbmcAdapter = (function() {
    var arr, button, method, _fn, _i, _len,
      _this = this;

    function XbmcAdapter(ip, port) {
      this.ip = ip;
      this.port = port;
      this.requests = {};
      this.id = 0;
      this.player = null;
      this.connect(this.ip, this.port);
      this.listen();
    }

    XbmcAdapter.prototype.connect = function(ip, port) {
      var _this = this;
      this.ws = new WebSocket('ws://' + ip + ':' + port + '/jsonrpc');
      return this.ws.addEventListener('open', function() {
        return _this.getActivePlayers();
      });
    };

    XbmcAdapter.prototype.sendRequest = function(data, cb) {
      this.id++;
      if (cb != null) {
        this.requests[this.id] = cb;
      }
      data.id = this.id;
      data.jsonrpc = '2.0';
      return this.ws.send(JSON.stringify(data));
    };

    XbmcAdapter.prototype.listen = function() {
      var _this = this;
      return this.ws.addEventListener('message', function(e) {
        var data, _ref, _ref1, _ref2;
        try {
          data = JSON.parse(e.data);
          if (data.result != null) {
            if (data.id !== void 0) {
              if (_this.requests[_this.id] !== void 0) {
                _this.requests[_this.id](data.result);
                delete _this.requests[_this.id];
              }
            }
          }
          switch (data.method) {
            case 'Player.OnPlay':
              return _this.player = (_ref = data.params) != null ? (_ref1 = _ref.data) != null ? (_ref2 = _ref1.player) != null ? _ref2.playerid : void 0 : void 0 : void 0;
            case 'Player.OnStop':
              return _this.player = null;
          }
        } catch (ex) {
          throw ex;
        }
      });
    };

    XbmcAdapter.prototype.getActivePlayers = function() {
      var _this = this;
      return this.sendRequest({
        method: 'Player.GetActivePlayers'
      }, function(data) {
        if (data.length > 0) {
          return _this.player = data[0].playerid;
        } else {
          return _this.player = null;
        }
      });
    };

    XbmcAdapter.prototype.sendPlayerRequest = function(data, cb) {
      if (data.params === void 0) {
        data.params = {};
      }
      if (this.player != null) {
        data.params.playerid = this.player;
      }
      return this.sendRequest(data, cb);
    };

    XbmcAdapter.prototype.sendPlayerButton = function(button, cb) {
      return this.sendPlayerRequest({
        method: 'Player.' + button
      }, cb);
    };

    XbmcAdapter.prototype.sendButton = function(method, button, cb) {
      return this.sendRequest({
        method: method + "." + button
      }, cb);
    };

    for (method in buttons) {
      arr = buttons[method];
      _fn = function(method, button) {
        var str;
        str = button.toLowerCase().substr(0, 1) + button.substr(1);
        return XbmcAdapter.prototype[str] = function(cb) {
          if (method === 'Player') {
            return this.sendPlayerButton(button, cb);
          } else {
            return this.sendButton(method, button, cb);
          }
        };
      };
      for (_i = 0, _len = arr.length; _i < _len; _i++) {
        button = arr[_i];
        _fn(method, button);
      }
    }

    return XbmcAdapter;

  }).call(this);

  window.Xbmc = XbmcAdapter;

}).call(this);
